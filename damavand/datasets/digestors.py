    ...
    def mine(self, mining_params):
        ...
        for file in self.files:
            ...
            for key in self.data.keys():
                temp_df = splitter(
                    mat_contents["Signal"][0][0][1][0][0][0][:, key],
                    mining_params["win_len"],
                    mining_params["hop_len"],
                )
                temp_df["load"], temp_df["state"], temp_df["severity"] = (
                    load,
                    state,
                    severity,
                )
    ...
    def __init__(self, base_directory, channels=["FE", "DE"]):
        ...
        self.data = {
            channel: {
                Fs: []
                for Fs in set(
                    [a.split(".")[0].split("_")[-1] for a in os.listdir(self.base_dir)]
                )
            }
                        ...
                        for channel in self.channels:
                            if channel in available_channels.keys():
                                ...
                                (
                                    ...
                                    processed_data["defected_bearing"],
                                    processed_data["severity"],
                                    processed_data["rot_speed"],
                                    processed_data["fs"],
        ...
        for folder in self.folders:
            if folder == "normal":
                ...
                for subfolder in os.listdir(self.base_dir + folder + "/"):
                    ...
                    for sev in os.listdir(
                        self.base_dir + folder + "/" + subfolder + "/"
                    ):
                        for file in os.listdir(
                            self.base_dir + folder + "/" + subfolder + "/" + sev + "/"
                        ):
                            df = pd.read_csv(
                                self.base_dir
                                + folder
                ...
                for sev in os.listdir(self.base_dir + folder + "/"):
                    for file in os.listdir(self.base_dir + folder + "/" + sev):
                        df = pd.read_csv(
                            self.base_dir + folder + "/" + sev + "/" + file, header=None
                        )
                        for key in self.data.keys():
                            temp_df = splitter(
                                df[key].values,
        ...
        for folder in self.folders:
            if folder.startswith("Healthy"):
                ...
                for file in os.listdir(self.base_dir + folder + "/"):
                    if file.endswith(".csv"):
                        load = (
                            file.split(" ")[1] + " " + file.split(" ")[1].split(".")[0]
                        )
                        ...
                        for i in self.data.keys():
                            ...
                            temp_df["state"], temp_df["severity"], temp_df["load"] = (
                                state,
                                severity,
                                load,
                            )
            ...
            if file.endswith(".mat"):
                ...
                if rep in self.reps:
                    ...
                    for channel in self.data.keys():
                        temp_df = splitter(
                            mat_data[channel].reshape((-1)),
                            mining_params["win_len"],
                            mining_params["hop_len"],
                        )
                        temp_df["state"], temp_df["loading"], temp_df["rep"] = (
                            state,
                            loading,
                            rep,
                        )
            ...
            for file in os.listdir(self.base_dir + folder):
                if file.endswith(".mat"):
                    if int(file.split(".")[0].split("_")[-1]) in self.reps:
                        rot_speed, load_torque, radial_force, code, rep = file.split(
                            "."
                        )[0].split("_")
                        try:
                            ...
                            if "CP1" in self.channels:
                                temp_df = splitter(
                                    mat_data[file.split(".")[0]]["Y"][0][0][0][1][
                                        2
                                    ].reshape((-1)),
                                    mining_params["win_len"],
                                    mining_params["hop_len"],
                            ...
                            if "CP2" in self.channels:
                                temp_df = splitter(
                                    mat_data[file.split(".")[0]]["Y"][0][0][0][2][
                                        2
                                    ].reshape((-1)),
                                    mining_params["win_len"],
                                    mining_params["hop_len"],
                            ...
                            if "Vib" in self.channels:
                                temp_df = splitter(
                                    mat_data[file.split(".")[0]]["Y"][0][0][0][6][
                                        2
                                    ].reshape((-1)),
                                    mining_params["win_len"],
                                    mining_params["hop_len"],
